...
const gulpSequence = require('gulp-sequence').use(gulp);//顺序执行或并行
const gutil = require('gulp-util');
const bower = require('gulp-bower');
const minimist = require('minimist');
const cache = require('gulp-cached');//可记录修改过的文件,利器
const plugins = require('gulp-load-plugins')();
const browserSync = require("browser-sync").create();
const rev = require('gulp-rev');
const revCollector = require('gulp-rev-collector');//html内容版本号替换
const config = require('../../../plugins/read-config.js');
...
gulp.task('copy-html', function () {
    return gulp.src(['./app/html/res/**/*.html'])
        .pipe(gulp.dest('app/html/rev/'));
});
gulp.task('copy-module',['copy-css','copy-sass','copy-img','copy-js','copy-html']);//复制自己的所有文件到rev目录中,用于生产环境测试及开发

gulp.task('copy-libs', function () {//复制第三方库文件
...
gulp.task('create-config-ng', function () { //生成angularjs 配置
    ...
    var isFind = false;
    var keyVals = [];
    for (var index in myConfig) {
        var mo = myConfig[index];
        if(mo.moduleName==moduleName){
            for (var name in mo) {
                var value = mo[name];
                keyVals.push("\"" + name + "\":"+"\"" + value + "\"");
            }
            isFind = true;
            break;
        }
    ...
    if(!isFind){
        throw (moduleName+" 模块在配置文件中找不到,请检查.");
    }
    cons+=keyVals.join(',');
    cons += "})});"
    return string_src("./_config-ng.js", cons)
        .pipe(gulp.dest('app/js/rev/'));
});
...
gulp.task('compress-img', function () {//压缩img
    return gulp.src('./app/img/rev/**/*.+(png|gif|jpg|svg)')
...
gulp.task('compress-html', function () {//压缩html
    ...
    return gulp.src('./app/html/rev/**/*.html')
...
gulp.task('version-html', function () {//生成html版本号
    return gulp.src(['app/**/rev/rev-manifest.json', 'app/html/rev/**/*.html'])
        .pipe(revCollector({
            replaceReved: true
        }))
        .pipe(gulp.dest('app/html/rev/'))
...
gulp.task('bs-watch',function () {
    return gulp.watch(['./app/html/rev/*.html','./app/css/rev/*.css','./app/sass/rev/*.css','./app/js/rev/**/*.js']).on('change',browserSync.reload);
});
...
gulp.task('watch-html', function() {
    return gulp.watch(['./app/html/res/**/*.html']).on('change',function () {
...
gulp.task('dev', gulpSequence('clean','bower', ['copy-module', 'copy-libs'], ['create-config-ng', 'build-css']));//开发环境打包配置
